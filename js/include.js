...
.then(data => {
    ...
    this.window.addEventListener("scroll",function(){
        ...
        //위에서 스크롤바가 얼만큼 내려왔는지 계산하여 변수에 저장한다.
    ...
    //하단에 마우스를 올리면 기존에 설정되어 있었던 2초 뒤에 사라지는 동작이 취소됨
    ...
    btn_admin.addEventListener("click",function(e){
        e.preventDefault()
        //a태그를 클릭했을 때 스크롤바가 위로 이동되는 기본 기능을 억제

        if(design_ver=="client"){
            document.body.classList.add("admin")
            design_ver = "admin"
            admin_img.setAttribute("src","/img/btn_delete_white.svg")
        }else{
            document.body.classList.remove("admin")
            design_ver ="client"
            admin_img.setAttribute("src","/img/btn_admin.svg")
        }
